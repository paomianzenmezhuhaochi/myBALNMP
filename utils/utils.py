...
def train_val_test_binary_class(task_type, epoch, model, data_loader, optimizer, recoder, writer, merge_method):
    ...
    # 用 tqdm 包装数据加载器
    pbar = tqdm(data_loader, desc=f"Epoch {epoch} ({task_type})", leave=True)

    if task_type == "train":
        model.train()
        for index, item in enumerate(data_loader, start=1):
            print(f"\repoch: {epoch}, {task_type}, [{index}/{len(data_loader)}]", end="")
            bag_tensor, label = item["bag_tensor"].to(device), item["label"].to(device)
            clinical_data = item["clinical_data"][0].to(device) if "clinical_data" in item else None
